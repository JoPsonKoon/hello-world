...
public class DateUtil {
	...
	public static void main(String[] args) {
		// 当前时间加一个月
		System.out.println(DateUtil.format(DateUtil.addTime(new Date(), 1, DateUnit.MONTH), ""));
		long[] time  = DateUtil.timeDifference(new Date(),  DateUtil.addTime(new Date(), 1, DateUnit.MONTH));
		System.out.println(time[0]+"天"+time[1]+"小时"+time[2]+"分"+time[3]+"秒");
	}
	 ...
	 * @return
	 */
	public static int hoursApart(Date start, Date end) {
		// 把时分秒忽略掉了
		Date s = parse(format(start, null),null);
		Date e = parse(format(end, null),null);

		long differentMillis = e.getTime() - s.getTime();

		long hours = differentMillis / (1000 * 60 * 60 );
	...
	public static  Date getDateTimeWithoutHms(Date date ){
		if (date == null) {
			return null;
		}
		Calendar calendar = toCalendar(date);
